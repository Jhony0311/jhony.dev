...
import { gardenEntries, type GardenStage, type GardenType } from "../home/home-data";
...
const STAGE_OPTIONS: readonly GardenStage[] = ["Seedling", "Budding", "Evergreen"];
const ALL_TAGS = Array.from(new Set(gardenEntries.flatMap((entry) => entry.tags))).sort();
...
const MATURITY_META: Record<GardenStage, { icon: typeof LeafIcon; className: string }> = {
...
function parseTypeFromPathname(pathname: string): FilterOption {
  ...
  const categorySegment = nextSegment === "category" ? segments[gardenIndex + 2] : nextSegment;
...
function parseStateFromLocation(pathname: string, search: string): GardenFilterState {
    ...
    <div className="flex flex-col gap-4">
      {[0, 1, 2].map((i) => (
        <div key={i} className="rounded-lg border border-line p-6">
          <div className="mb-4 h-5 w-[60%] animate-shimmer rounded bg-canvas-inset" />
          <div className="mb-3 h-3.5 w-full animate-shimmer rounded bg-canvas-inset" />
          <div className="h-3 w-[85%] animate-shimmer rounded bg-canvas-inset" />
        </div>
      ))}
    </div>
  );
...
function TagFilterGroup({
  ...
  const hiddenTagsCount = Math.max(0, selectedTags.length - visibleTags.length);
...
export function GardenPage({
  ...
  const [selectedType, setSelectedType] = useState<FilterOption>(initialSelectedType);
  const [selectedStage, setSelectedStage] = useState<FilterOption>(initialSelectedStage);
  ...
  const filteredEntries = useMemo(() => {
    return gardenEntries.filter((entry) => {
      const typePass = selectedType === ALL_FILTER || entry.type === selectedType;
      const stagePass = selectedStage === ALL_FILTER || entry.stage === selectedStage;
      const tagPass =
  ...
  useEffect(() => {
    const onPopState = () => {
      const state = parseStateFromLocation(window.location.pathname, window.location.search);
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 border-b border-line pb-8 md:grid-cols-3">
        ...
        <FilterGroup
          ...
          renderOption={(option) => {
            ...
            const { icon: StageIcon, className } = MATURITY_META[stage];

            return (
              <span className="inline-flex items-center gap-1.5">
                <span>{option}</span>
                <StageIcon size={12} weight="fill" className={className} />
              </span>
            );
          }}
        />
